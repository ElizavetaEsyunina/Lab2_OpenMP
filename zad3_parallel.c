...
int main() {
        ...
        int i, j, array_size, num_iterations, num_threads;
        clock_t start, end;
        ...
        if (num_threads_str == NULL) {
                fprintf(stderr, "Error: OMP_NUM_THREADS environment variable not set.\n");
                return 1;
        }
        num_threads = strtol(num_threads_str, &endptr, 10);
        if (*endptr != '\0' || num_threads <= 0) {
                fprintf(stderr, "Error: Incorrect OMP_NUM_THREADS value: %s\n", num_threads_str);
                return 1;
        }
