...
int main() {
        ...
        if (num_threads_str == NULL) {
                fprintf(stderr, "Error: NUM_THREADS environment variable not set. \n");
                return 1;
        }

        num_threads = strtol(num_threads_str, &endptr, 10);
        if (*endptr != '\0' || num_threads <= 0) {
                fprintf(stderr, "Error: Incorrect NUM_THREADS value: %s\n", num_threads_str);
                return 1;
        }
        ...
        quotient = (double **)malloc(rows * sizeof(double *));
